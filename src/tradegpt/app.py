...
def _scan_input(request: ScanRequest) -> ScanInput:
    return ScanInput(symbol=request.symbol, discovered_at=request.discovered_at, catalyst_score=request.catalyst_score, technical_score=request.technical_score, relative_strength_score=request.relative_strength_score, liquidity_score=request.liquidity_score, last_price=request.last_price, entry_trigger=request.entry_trigger, stop_price=request.stop_price, target_price=request.target_price, data_verified=request.data_verified, adv_shares=request.adv_shares, adv_dollars=request.adv_dollars, trigger_confirmed=request.trigger_confirmed)


def _risk_payload(risk) -> dict | None:
    if risk is None:
        return None
    return {"approved": risk.approved, "shares": risk.shares, "risk_dollars": risk.risk_dollars, "reward_risk": risk.reward_risk, "reasons": list(risk.reasons)}


def _learning_payload(record_id: int, record: LearningRecord) -> dict:
    outcome = record.outcome
    return {"id": record_id, "symbol": record.symbol, "discovered_at": record.discovered_at, "discovery_score": record.discovery_score, "discovery_state": record.discovery_state.value, "trigger_confirmed": record.trigger_confirmed, "trade_ready": record.trade_ready, "traded": record.traded, "missed_opportunity": record.missed_opportunity, "reasons": list(record.reasons), "outcome": None if outcome is None else {"symbol": outcome.symbol, "evaluated_at": outcome.evaluated_at, "entry_price": outcome.entry_price, "exit_price": outcome.exit_price, "stop_price": outcome.stop_price, "target_price": outcome.target_price, "outcome_r": outcome.outcome_r, "max_adverse_excursion_r": outcome.max_adverse_excursion_r, "max_favorable_excursion_r": outcome.max_favorable_excursion_r, "result": outcome.result}}
...
def _learning_summary(records: list[LearningRecord]) -> dict[str, float | int]:
    ...
    return {"discoveries": len(records), "trade_ready": sum(r.trade_ready for r in records), "traded": sum(r.traded for r in records), "missed_opportunities": sum(r.missed_opportunity for r in records), "resolved_outcomes": len(rs), "win_rate_pct": round(len(wins) / len(rs) * 100, 2) if rs else 0.0, "average_r": round(sum(rs) / len(rs), 3) if rs else 0.0, "total_r": round(sum(rs), 3), "average_win_r": round(sum(wins) / len(wins), 3) if wins else 0.0, "average_loss_r": round(sum(losses) / len(losses), 3) if losses else 0.0}


def _scan_run_payload(run: ScanRun) -> dict:
    return {"scan_id": run.scan_id, "scheduled_at": run.scheduled_at, "started_at": run.started_at, "completed_at": run.completed_at, "status": run.status, "error": run.error}
...
def process_scan(request: ScanRequest) -> dict:
    ledger = AuditLedger()
    result = ScanOrchestrator(lifecycle=CandidateLifecycle(ledger)).process(_scan_input(request), equity=request.equity, current_heat=request.current_heat, daily_loss=request.daily_loss, exceptional=request.exceptional)
    persisted = store.upsert(result.candidate)
    for event in ledger.all():
        audit_store.append(event)
    return {"candidate": candidate_payload(persisted), "risk_decision": _risk_payload(result.risk_decision), "execution_reasons": list(result.execution_reasons)}
...
def scheduler_status(now: datetime | None = Query(default=None)) -> dict:
    ...
    return {"timezone": status.timezone, "next_run": None if status.next_run_at is None else {"scan_id": status.next_scan_id, "scheduled_at": status.next_run_at}, "scans": list(status.scans)}


@app.get("/api/v1/scheduler/due")
def scheduler_due(now: datetime | None = Query(default=None)) -> dict:
    moment = now or datetime.now(timezone.utc)
    return {"evaluated_at": moment, "due": [{"id": schedule.id, "label": schedule.label, "time_et": schedule.time_et.isoformat(timespec="minutes")} for schedule in scheduler_service.due(moment)]}
...
def mark_learning_triggered(record_id: int) -> dict:
    record = learning_store.get(record_id)
    if record is None: raise HTTPException(status_code=404, detail="learning record not found")
...
def record_learning_outcome(record_id: int, request: LearningOutcomeRequest) -> dict:
    record = learning_store.get(record_id)
    if record is None: raise HTTPException(status_code=404, detail="learning record not found")
    if record.outcome is not None: raise HTTPException(status_code=409, detail="learning record already has an outcome")
    ...
    if outcome_r is None and request.entry_price is not None and request.exit_price is not None and request.stop_price is not None:
        ...
        if risk_per_share > 0: outcome_r = (request.exit_price - request.entry_price) / risk_per_share
...
def system_status() -> dict[str, object]:
    ...
    return {"service": "tradegpt-v2", "mode": "research", "live_execution_enabled": False, "options_enabled": False, "zero_dte_enabled": False, "broker_orders_enabled": False, "candidate_count": len(candidates), "state_counts": state_counts, "learning": learning_summary(), "scheduler": scheduler_status(), "timestamp": datetime.now(timezone.utc).isoformat()}
