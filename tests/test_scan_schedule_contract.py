...
def test_production_scan_schedule_contract():
    ...
    assert [item.scan_id for item in schedule]==["daily-discovery","primary-qualification","late-day-discovery"]
...
def test_schedule_does_not_embed_order_execution():
    assert all("order" not in item.scan_id.lower() for item in SchedulerService.production_schedule())